...
if ( ! function_exists( 'astra_comment_form_default_markup' ) ) {
	...
	function astra_comment_form_default_markup( $args ) {
		...
		if ( 'post' == get_post_type() || $all_post_type_support ) {
			...
			$args['comment_field']     = '<div class="ast-row comment-textarea"><div class="comment-form-comment"><div class="comment-form-textarea ast-col-lg-12"><label for="comment" class="screen-reader-text">' . esc_html( astra_default_strings( 'string-comment-label-message', false ) ) . '</label><textarea id="comment" name="comment" placeholder="' . esc_attr( astra_default_strings( 'string-comment-label-message', false ) ) . '" cols="45" rows="' . ( 'modern-skin' === Astra_Skins::astra_get_selected_skin() ? '6' : '8' ) . '" aria-required="true"></textarea></div></div></div>';
		}
