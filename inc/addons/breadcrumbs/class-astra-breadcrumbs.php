...
if ( ! class_exists( 'Astra_Breadcrumbs' ) ) {
	...
	class Astra_Breadcrumbs {
		 ...
		 * @since 1.7.0
		 */
		public function astra_breadcrumb() {
			...
			$items = Astra_Breadcrumbs::astra_breadcrumb_get_items( $args );
			if ( ! empty( $items ) ) {
		 ...
		 * @since 1.7.0
		 */
		private function astra_breadcrumb_get_items( $args ) {
			global $wp_query;
			$item          = array();
			...
			if ( is_front_page() && is_home() ) {
				...
				if ( 'page' !== $post_type ) {
					...
					if ( isset( $args[ "singular_{$post_type}_taxonomy" ] ) && is_taxonomy_hierarchical( $args[ "singular_{$post_type}_taxonomy" ] ) ) {
						...
						if ( isset( $terms[0] ) ) {
							$item = array_merge( $item, Astra_Breadcrumbs::astra_breadcrumb_get_term_parents( $terms[0], $args[ "singular_{$post_type}_taxonomy" ] ) );
						}
				...
				$parents     = Astra_Breadcrumbs::astra_breadcrumb_get_parents( $post_parent );
				...
				if ( is_category() || is_tag() || is_tax() ) {
					...
					$parents  = Astra_Breadcrumbs::astra_breadcrumb_get_term_parents( $term->parent, $term->taxonomy );
		 ...
		 */
		private function astra_breadcrumb_get_bbpress_items( $args = array() ) {
			$item             = array();
			...
			if ( bbp_is_forum_archive() ) {
				...
				$item     = array_merge( $item, Astra_Breadcrumbs::astra_breadcrumb_get_parents( bbp_get_reply_topic_id( $reply_id ) ) );
				...
				if ( 0 !== $forum_parent_id ) {
					$item = array_merge( $item, Astra_Breadcrumbs::astra_breadcrumb_get_parents( $forum_parent_id ) );
				}
		 ...
		 */
		private function astra_breadcrumb_get_parents( $post_id = '', $delimiter = '/' ) {
			$parents = array();
			if ( 0 == $post_id ) {
				return $parents;
			}
		 ...
		 */
		private function astra_breadcrumb_get_term_parents( $parent_id = '', $taxonomy = '', $delimiter = '/' ) {
			$html    = array();
			$parents = array();
